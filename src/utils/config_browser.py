...
def load_config():
    ...
    try:
        ...
        with open(config_path, "r") as file:
            ...
            # Ensure CAPTCHA has all required fields
            if "CAPTCHA" in config:
                ...
                for key, default_value in captcha_defaults.items():
                    if key not in config["CAPTCHA"]:
                        config["CAPTCHA"][key] = default_value

            # Ensure RPCS has NEURAVERSE field
            if "RPCS" in config:
                rpcs_defaults = {"NEURAVERSE": ["https://testnet.rpc.neuraprotocol.io"]}
            ...
            # Ensure ZOTTO has required fields
            if "ZOTTO" not in config:
                config["ZOTTO"] = {}
            ...
            for key, default_value in zotto_defaults.items():
                if key not in config["ZOTTO"]:
                    config["ZOTTO"][key] = default_value

            # Ensure BRIDGE has required fields
            ...
            for key, default_value in bridge_defaults.items():
                if key not in config["BRIDGE"]:
                    config["BRIDGE"][key] = default_value
...
def save_config(config):
    ...
    try:
        ...
        # Убедимся, что раздел CAPTCHA содержит все необходимые поля
        ...
        # Убедимся, что раздел ZOTTO содержит все необходимые поля
        if "ZOTTO" not in config:
            config["ZOTTO"] = {}
        ...
        # Убедимся, что раздел BRIDGE содержит все необходимые поля
        if "BRIDGE" not in config:
            config["BRIDGE"] = {}
...
def open_browser():
    ...
    try:
        webbrowser.open("http://127.0.0.1:3456")
...
<head>
    ...
    <title>StarLabs Neuraverse Configuration</title>
    ...
    <div class="app-container">
        <header>
            <div class="logo">
                ...
                <h1>StarLabs Neuraverse Configuration</h1>
            </div>
        ...
        <main>
            <div class="sidebar">
                <div class="sidebar-menu">
                    ...
                    <div class="sidebar-item" data-section="zotto">
                        <i class="fas fa-retweet"></i>
                        <span>Zotto</span>
                    </div>
                    <div class="sidebar-item" data-section="bridge">
...
function renderConfig(config) {
    ...
    const sections = {
        ...
        'zotto': { key: 'ZOTTO', title: 'Zotto', icon: 'retweet' },
        'bridge': { key: 'BRIDGE', title: 'Bridge', icon: 'bridge' },
    ...
    Object.entries(sections).forEach(([sectionId, { key, title, icon }], index) => {
        ...
        if (config[key]) {
            if (key === 'SETTINGS') {
                ...
                // Карточка для настроек Zotto
                createCard(cardsContainer, 'Zotto Swap Settings', 'retweet', [
                    { key: 'BALANCE_PERCENT_TO_SWAP', value: config[key]['BALANCE_PERCENT_TO_SWAP'] },
                    { key: 'NUMBER_OF_SWAPS', value: config[key]['NUMBER_OF_SWAPS'] }
                ], key);
            } else if (key === 'BRIDGE') {
                // Карточка для настроек Bridge
                createCard(cardsContainer, 'Bridge Settings', 'bridge', [
                    { key: 'SEPOLIA_BALANCE_PERCENT_TO_BRIDGE', value: config[key]['SEPOLIA_BALANCE_PERCENT_TO_BRIDGE'] },
                    { key: 'ANKR_BALANCE_PERCENT_TO_BRIDGE', value: config[key]['ANKR_BALANCE_PERCENT_TO_BRIDGE'] },
                    { key: 'BRIDGE_ALL_TO_SEPOLIA', value: config[key]['BRIDGE_ALL_TO_SEPOLIA'] },
                    { key: 'BRIDGE_ALL_TO_ANKR', value: config[key]['BRIDGE_ALL_TO_ANKR'] }
                ], key);
            } else if (key === 'OTHERS') {
                // Остальные категории
...
def run():
    ...
    try:
        ...
        logger.info("Configuration interface available at: http://127.0.0.1:3456")
        logger.info("To exit and return to main menu: Press CTRL+C")
