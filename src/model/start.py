...
from src.model.neuraverse.instance import Neuraverse
...
class Start:
    def __init__(
        ...
        self.neuraverse_web3: Web3Custom | None = None
        self.neuraverse_instance: Neuraverse | None = None
    ...
    async def initialize(self):
        try:
            ...
            self.neuraverse_web3 = await Web3Custom.create(
                self.account_index,
                self.config.RPCS.NEURAVERSE,
                self.config.OTHERS.USE_PROXY_FOR_RPC,
                self.proxy,
                self.config.OTHERS.SKIP_SSL_VERIFICATION,
            )

            self.neuraverse_instance = Neuraverse(
                self.account_index,
                self.session,
                self.neuraverse_web3,
                self.config,
                self.wallet,
                self.proxy,
                self.discord_token,
            ...
            if not await self.neuraverse_instance.login():
                return False
    ...
    async def flow(self):
        try:
            try:
                wallet_stats = WalletStats(self.config, self.neuraverse_web3)
            ...
            if not tasks:
                ...
                if self.neuraverse_web3:
                    await self.neuraverse_web3.cleanup()
                return True
            ...
            if self.config.SETTINGS.SEND_TELEGRAM_LOGS:
                message = (
                    f"🐰 Neuraverse StarLabs Bot Report\n\n"
    ...
    async def execute_task(self, task):
        ...
        if task == "connect_socials":
            return await self.neuraverse_instance.connect_socials()

        elif task == "complete_leaderboard_quests":
            return await self.neuraverse_instance.complete_quests()

        elif task == "zotto_swaps":
            return await self.neuraverse_instance.perform_swaps()

        elif task == "neura_bridge":
            return await self.neuraverse_instance.perform_bridge()
